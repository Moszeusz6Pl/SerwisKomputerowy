...
class KlientDTO
{
    ...
    function Edytuj($id, $Login, $Imie, $Nazwisko, $Adres, $Email, $Telefon)
    {
        ...
        $query = 'SELECT COUNT(*) FROM KLIENCI WHERE idKlienta='.$id;
        if(($odpowiedz=mysql_query($query))==false)
           return('Baza danych nie odpowiada na zapytanie '. $query);
        
        $result=mysql_fetch_row($odpowiedz);
        if($result[0]!=1)
            return('Nie ma użytkownika o id '.$id.' lub jest wielu o tym samym id');
